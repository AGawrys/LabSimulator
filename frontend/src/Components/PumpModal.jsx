...
class PumpModal extends React.Component {
	...
	render() {
		...
		return (
			<Modal show={show} onHide={this.onHide} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
				...
				<Modal.Body id="blend-body" style={{ height: '500px', width: '750px' }}>
					<Row style={{ display: 'flex', alignItems: 'flex-end' }}>
						<Tool tool={targetCopy} />
						<Tool tool={sourceCopy} />
					</Row>
					<Button
						variant="primary"
					...
					<Button variant="primary" disabled={finished || earlyRelease || earlyPump} onClick={this.onFinish}>
						Finish
					</Button>
				</Modal.Body>
				<Modal.Footer>
	...
	onPump() {
		...
		if (compressed < 1) {
			sourceCopy.image.animation.compressed += 0.05;
			targetCopy.image.properties.Fill += 0.0025;
			this.setState({
				sourceCopy,
				targetCopy
			});
	...
	onPumpEnd() {
		...
		if (compressed > 0) {
			sourceCopy.image.animation.compressed -= 0.25;
		} else {
